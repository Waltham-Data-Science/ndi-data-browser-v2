...
- **PR-branch freshness is gated by CI, not by branch protection.** GitHub's "Require branches up to date before merging" setting is paywalled for private repos on the Free plan (needs GitHub Team or going public). `.github/workflows/pr-branch-freshness.yml` is the best-effort substitute — it fails with a clear error when a PR's base is behind `origin/main`, which puts a red status on the PR. CI red is a strong visible signal but NOT a hard merge-block without the paid tier. If someone (or an agent) ignores red CI and merges anyway, stale-base merges are still possible. Upgrade path: GitHub Team (~$4/user/mo) unlocks `required_status_checks` on the API, OR make the repo public. The latent-conflict pattern that motivated this gate is documented in fix-main PR #23 (2026-04-18).
