...
- [docs/adr/](docs/adr/) — 6 ADRs (proxy backend, session cookies, Redis, dropping SQLite, refresh tokens, React Router)
...
3. **Services are pure business logic.** They never import `fastapi` and never do HTTP directly. Cloud I/O lives in `clients/ndi_cloud.py`.
...
make lint
make build           # frontend + docker image
```
...
- `backend/tests/unit/` — 53 tests across error catalog, cloud client, circuit breaker, session store, CSRF, rate limiter, projection, query validation, cache
